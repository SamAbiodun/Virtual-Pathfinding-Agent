...
public class AStar
{
    ...
    public List<Connection> PathfindAStar(Graph aGraph, GameObject start, GameObject end, Heuristic myHeuristic)
    {
        ...
        while (OpenList.GetSize() > 0)
        {
            ...
            foreach (Connection aConnection in Connections)
            {
                ...
                else
                {
                    ...
                    // We’ll need to calculate the heuristic value using the function, since we don’t an existing record to use.
        ...
        List<Connection> tempList = new List<Connection>(); if (!CurrentRecord.Node.Equals(end))
        {
